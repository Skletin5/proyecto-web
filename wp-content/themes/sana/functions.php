 ...
 */
if ( !function_exists( 'sana_get_template_part' ) ) {

	function sana_get_template_part( $slug, $name = null, array $sana_params = array() ) {
 ...
 * Print H1 header
*/
if ( !function_exists( 'sana_the_h1' ) ) {

	function sana_the_h1() {

		$title = sana_get_the_h1();
		if ( !empty($title) ) echo '<h1>' . esc_html( $title ) . '</h1>';
	}
}

if ( !function_exists( 'sana_get_the_h1' ) ) {

	function sana_get_the_h1() {

		if ( is_home() ) {

			$title = esc_html__( 'All Blog Posts', 'sana' );
		} 
			else
		if ( is_front_page() ) {

			$title = esc_html__( 'Home', 'sana' );
		}
			else
		if ( is_year() ) {

			$title = sprintf( esc_html__( 'Year Archives: %s', 'sana' ), get_the_date( 'Y' ) );
		}
			else				
		if ( is_month() ) {

			$title = sprintf( esc_html__( 'Month Archives: %s', 'sana' ), get_the_date( 'F Y' ) );
		}
			else
		if ( is_day() ) {

			$title = sprintf( esc_html__( 'Day Archives: %s', 'sana' ), get_the_date() );
		}
			else
		if ( is_category() ) {
		...
		if ( is_tag() ) {
			...
			$title = sprintf( esc_html__( 'Tag: %s', 'sana' ), single_tag_title( '', false ) );
		}
			else
		if ( is_tax() ) {
		...
		if ( is_search() ) {
			...
			$title = sprintf( esc_html__( 'Search Results: %s', 'sana' ), get_search_query() );
		} 
			else				
		if ( is_author() ) {
			...
			$title = sprintf( esc_html__( 'Author: %s', 'sana' ), $q->display_name );
		} 
			else
		...
		if ( is_attachment() ) {
			...
			$title = sprintf( esc_html__( 'Attachment: %s', 'sana' ), get_the_title() );
		}
			else
		if ( is_404() ) {

			$title = esc_html__( '404 Not Found', 'sana' );
		}
			else {
 ...
 */
if ( !function_exists( 'sana_add_current_nav_class' ) ) {

	function sana_add_current_nav_class( $classes, $item ) {
...
add_action( 'nav_menu_css_class', 'sana_add_current_nav_class', 10, 2 );
...
if ( !function_exists( 'sana_excerpt' ) ) {
	
	function sana_excerpt( $content, $excerpt = 0 ) {
		...
		if ( ! empty( $post->post_content ) &&
			...
			$content = sana_cut_excerpt( $post->post_content , $excerpt );
		}
...
if ( !function_exists( 'sana_cut_excerpt' ) ) {
	
	function sana_cut_excerpt( $content = '', $excerpt = 0 ) {

		$cut = false;
		$excerpt_more = apply_filters( 'excerpt_more', ' ...' );
		$content = sana_get_content( $content );
		...
		$content = sana_strip_tags( $texts, $cut );
 ...
 */
if ( !function_exists( 'sana_cut_text' ) ) {

	function sana_cut_text( $text, $cut = 300, $aft = ' ...' ) {
 ...
 */
if ( !function_exists( 'sana_get_content' ) ) {
	
	function sana_get_content( $content = '' ) {
...
if ( !function_exists( 'sana_strip_tags' ) ) {
	
	function sana_strip_tags( $texts = array(), $cut = 0 ) {
 ...
 * @return bool
 */
if ( !function_exists( 'sana_is_wc' ) ) {

	function sana_is_wc($tag, $attr='') {
...
if ( !function_exists( 'v' ) ) {
	...
	function sana_plugin_is_active( $plugin_var, $plugin_dir = null ) {
 ...
 */
if ( !function_exists( 'sana_admin_css' ) ) {
	
	function sana_admin_css() {

		wp_enqueue_style( 'sana-admin-css', get_template_directory_uri() . '/assets/css/admin.css', false, '1.0.0' );
	}
}
add_action( 'admin_enqueue_scripts', 'sana_admin_css' );

/**
 * Return inverted contrast value of color
 */
if ( !function_exists( 'sana_rgb_contrast' ) ) {
	
	function sana_rgb_contrast($r, $g, $b) {
 ...
 */
if ( !function_exists( 'sana_color_change' ) ) {
	
	function sana_color_change( $hex, $percent ) {
...
function sana_adjustBrightness($hex, $steps) {
 ...
 * @return array();
 */
if ( !function_exists( 'sana_get_footer_cols_num' ) ) {

	function sana_get_footer_cols_num() {
 ...
 */
if ( !function_exists( 'sana_get_navbar_layout' ) ) {

	function sana_get_navbar_layout( $default = null ) {
		...
		if ( ( !empty( $navbar_layout) AND empty( $sana_theme_config['navbar'][$navbar_layout] ) )
 ...
 * Return navbar menu
*/
if ( !function_exists( 'sana_get_wp_nav_menu' ) ) {

	function sana_get_wp_nav_menu() {
 ...
 */
if ( !function_exists( 'sana_get_sections' ) ) {

	function sana_get_sections() {
 ...
 */
if ( !function_exists( 'sana_get_pageheader_layout' ) ) {

	function sana_get_pageheader_layout() {
 ...
 */
if ( !function_exists( 'sana_get_pageheader_class' ) ) {

	function sana_get_pageheader_class() {
		
		$sana_header_class = array();
		$sana_h1 = sana_get_the_h1();

		if ( !empty($sana_h1) ) {

			$sana_header_class[] = ' header-h1 ';
		}
		...
		if ( function_exists('FW') ) {
			...
			if ( $header_fixed == 'fixed' ) {
				...
				$sana_header_class[] = ' header-parallax ';
			}
		}

		if ( function_exists( 'bcn_display' ) && !is_front_page() ) {

			$sana_header_class[] = ' hasBreadcrumbs';
		}

		$navbar_layout = 'white';
		if ( function_exists( 'FW' ) ) {

			$navbar_layout = sana_get_navbar_layout('white');
		}

		$sana_header_class[] = ' wrapper-navbar-layout-' . $navbar_layout;



		return implode( ' ', $sana_header_class );
	}
}
 ...
 */
if ( !function_exists( 'sana_get_pageheader_wrapper' ) ) {

	function sana_get_pageheader_wrapper() {
 ...
 */
if ( !function_exists( 'sana_check_active_sidebar' ) ) {

	function sana_check_active_sidebar() {

		if ( sana_is_wc('woocommerce') || sana_is_wc('shop') || sana_is_wc('product') ) {
 ...
 */
if ( !function_exists( 'sana_get_wc_sidebar_pos' ) ) {

	function sana_get_wc_sidebar_pos() {

		if ( sana_is_wc('product') ) {
		...
		if ( function_exists( 'FW' ) ) {
			...
			if ( sana_is_wc('product') ) {
				...
				$sana_sidebar = fw_get_db_settings_option( 'shop_post_sidebar' );
			}	
				else {

				$sana_sidebar = fw_get_db_settings_option( 'shop_list_sidebar' );
			}
 ...
 */
if ( !function_exists( 'sana_custom_css' ) ) {

	function sana_custom_css( $css = null ) {
 ...
 */
if ( !function_exists( 'sana_find_http' ) ) {

	function sana_find_http( $string ) {
 ...
 * 
 */
function sana_the_img_srcset( $attachment_id, $sizes_hooks, $sizes_media ) {
