...
if ( !function_exists('sana_get_google_fonts') ) {

	function sana_get_google_fonts($css = array()) {
		...
		foreach ( array('font_main', 'font_headers', 'font_subheaders') as $type ) {
			...
			if ( !empty($font) ) {
				...
				if ( !empty($font_weights) ) {
					...
					$sana_items = explode(',', $font_weights);
					foreach ( $sana_items as $item) {
		...
		if ( !empty($google_fonts) ) {
			...
			wp_enqueue_style( 'sana-google-fonts', esc_url( add_query_arg( $query_args, '//fonts.googleapis.com/css' ) ), array(), null );
		}
