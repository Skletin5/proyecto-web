	...
	private function do_import(array $args, array $state) {
		 ...
		 */
		{
			...
			// Prepare clean search->replace strings
			{
				...
				$search_replace['https://themeforest.net/user/like-themes'] = 'https://like-themes.com';
				$search_replace['https://themeforest.net/user/just-themes'] = 'https://like-themes.com';
...
/*
				$wpdb->query($wpdb->prepare(
					"DELETE FROM {$wpdb->options} WHERE option_name = 'new_admin_email'"
				));
*/
				$wpdb->query($wpdb->prepare(
					"DELETE FROM {$tmp_options_table} WHERE option_name = 'new_admin_email'"
				));
