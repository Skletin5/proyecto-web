	...
	private function get_next_files($root_dir, array &$dirs, $previous_file, array $exclude_paths) {
		...
		if ($names = array_diff(($names = scandir($dir = $root_dir . $rel_dir)) ? $names : array(), array('.', '..'))) {
			...
			foreach ($names as $file) {
				...
				if ( str_contains($file, 'woocommerce-placeholder') ) continue;
				if ( substr($file, -3) == 'dat' OR 
					 substr($file, -3) == 'log' OR 
					 substr($file, -4) == 'json' )  continue;				

				if (is_dir($path)) {

					// Skipping everything except media files dir
					if ( !ctype_digit(substr($path, -2)) ) {

						continue;
					} 					
					elseif (isset($exclude_paths[ fw_fix_path($path) ])) {
						continue;
