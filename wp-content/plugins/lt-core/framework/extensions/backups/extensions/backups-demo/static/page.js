...
jQuery(function($){
	var inst = {
		...
		checkActivationStatus: function(){
			...
			.done(function(r){
				if (r.success) {
					inst.addStatusLabel(r.data.activated);
				}
			});
		},
		addStatusLabel: function(isActivated){
			var $label = $('<div class="lt-theme-status-label"></div>');
			
			if (isActivated) {
				$label.html('<span class="dashicons dashicons-yes-alt"></span> Theme Activated');
				$label.addClass('activated');
			} else {
				$label.html('<span class="dashicons dashicons-warning"></span> Theme Not Activated');
				$label.addClass('not-activated');
				$label.append(' <a href="' + ajaxurl.replace('admin-ajax.php', 'admin.php?page=lt-activation') + '" class="button button-small button-success">Activate Now</a>');
			}
			
			$('.wrap h2').after('<p></p>').next('p').after($label);
		}
	};
